...
import torch
from ergt_phi.shadow_phase import CalibrationConfig,PhaseAnchorNetwork,ShadowPhaseModel,calibration_step,readiness
from ergt_phi.shadow_data import training_only,pair_partition,cache_training_fields,pack_records,feature_conflicts
...
def main():
    ...
    phase = PhaseAnchorNetwork(base.config.hidden_dim,base.config.n_worlds,3,cfg)
    shadow = ShadowPhaseModel(base,phase)
    ...
    files = ['ergt_phi/shadow_phase.py','ergt_phi/shadow_data.py','ergt_phi/shadow_metrics.py',
             'ergt_phi/checkpoint.py','ergt_phi/native_steps.py','scripts/run_m2.py']
    contract = {'stage':'M2_shadow_only','config':asdict(cfg),'base_checkpoint_sha256':digest(cp),
    ...
    print(f'Training hash verified. Fit={len(fit)}, monitor={len(monitor)}; caching frozen Psi0.',flush=True)
    records = cache_training_fields(shadow,examples,tokenizer)
    ...
    with torch.no_grad():
        ...
        exact(initial[:,:,[edge.source_position,edge.target_position]],counter[:,:,[edge.source_position,edge.target_position]])
    ...
    with torch.no_grad():
        ...
        exact(trained_anchors[:,:,[edge.source_position,edge.target_position]],changed_anchors[:,:,[edge.source_position,edge.target_position]])
    exact(base.state_dict(),baseline_state)
    assert all(not p.requires_grad and p.grad is None for p in base.parameters())
    counterexample = {'remote_relation_token_changed':True,'canonical_endpoint_anchors_exactly_unchanged_before_and_after_training':True,
                      'interpretation':'Per-token Psi0 anchors cannot distinguish this change of relation at canonical endpoints.'}
    if qualified:
