...
def main():
    ...
    out=ROOT/'runs/m2_proposal';out.mkdir(parents=True,exist_ok=True);run=ROOT/'runs/imported_m0/m0_single_seed_reference'
    ...
    (out/'result.json').write_text(json.dumps(result,indent=2));(ROOT/'manifests/m2_proposal.json').write_text(json.dumps(result,indent=2));print(json.dumps({k:v['final_monitor'] for k,v in results.items()},indent=2))
