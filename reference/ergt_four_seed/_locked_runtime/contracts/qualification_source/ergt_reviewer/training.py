"""Preregistered training loops for the two primary model arms."""
...
from .fair_data import partition_mechanism_tuning_shards
...
def _load_training_state(
    ...
    *,
    ...
    if state.get("run_hash") != run_hash:
        print(f"[reviewer-resume] ignored incompatible checkpoint: {path}")
        return 0, None, float("-inf"), [], {}
    ...
    *,
    ...
    for step in step_iterator:
        ...
        if (
            ...
            print(
                f"[reviewer-native] step={step} teacher={teacher:.1f} "
...
def _save_field_candidate(
    ...
    *,
    ...
    torch.save(
        {
            "schema_version": "ergt-reviewer-v6-frozen-field-candidate-v1",
            "run_hash": run_hash,
            "step": int(step),
    ...
    *,
    ...
    for step in step_iterator:
        ...
        if should_eval:
            ...
            print(
                f"[reviewer-native-v6] step={step} teacher={teacher:.1f} "
                f"loss={train_loss:.4f} tune={metrics['accuracy']:.3f} "
                f"pair={metrics['counterfactual_pair_exact']:.3f} "
                f"measure={_measurement_min(metrics):.3f} "
                f"actionMAE={metrics['effective_action_mae']:.3f} "
    ...
    *,
    ...
    for step in step_iterator:
        ...
        if step == 1 or step == steps or step % eval_interval == 0:
            ...
            print(
                f"[reviewer-transformer] step={step} loss={float(loss.detach().cpu()):.4f} "
    ...
    if not training_complete and stage == "warmup":
        for current in range(stage_step + 1, warmup_steps + 1):
            ...
            if current == 1 or current == warmup_steps or current % eval_interval == 0:
                ...
                print(
                    f"[reviewer-transformer-v7] candidate={candidate_id} "
    ...
    if not run_terminated and phase == "warmup":
        for current in range(phase_step + 1, warmup_steps + 1):
            ...
            if current == 1 or current == warmup_steps or current % eval_interval == 0:
                ...
                print(
                    f"[reviewer-transformer-v8] candidate={candidate_id} "
                    f"stage=warmup step={current} loss={float(loss.detach().cpu()):.4f} "
                    f"id={id_metrics['supported_accuracy']:.3f} "
                    f"pair={id_metrics['counterfactual_pair_exact']:.3f} "
                    f"stable={warmup_streak}/{required_windows}"
...
def train_models(
    ...
    *,
    ...
    native_trainer = (
        _train_native_decoupled_v6
        if checkpoint_protocol == "v21_decoupled_frozen_v6"
        else _train_native
    )
    native_curves, native_resource = native_trainer(
        native,
        train,
        tuning,
        mechanism_tuning,
